...
$(document).ready(function() {
	...
	function megamenu_sizer(){
		...
		for ( var k=0 ; k < megaBins.length ; k++ ) {
			...
			var i = 0;
            var totalMegaSlots = (megaSlots.length < 4) ? megaSlots.length : 4;

			if(megaBins.eq(k).hasClass('megamenu-slots-columnar')) {
				var slotTotalWidth = 0;
				for ( i=0 ; i < totalMegaSlots ; i++ ) {
			...
			var edgeOverlap = ($(window).width() - (megaBins.eq(k).offset().left + megaBins.eq(k).outerWidth(true))) - 100;
...
;
Drupal.behaviors.unionSuperfish = function (context) {
...
Drupal.behaviors.unionSuperfishBlocks = function (context) {
...
Drupal.behaviors.unionHighlightParentMenu = function (context) {

    $("div.block ul.sf-menu li ul li").mouseover(function() {
        $(this).parent().parent().addClass("menu-highlight");
    });

    $("div.block ul.sf-menu li ul li").mouseout(function() {
        $(this).parent().parent().removeClass("menu-highlight");
    });
};

$(document).ready(function() {

    var colors = ['orange', 'blue', 'red', 'green', 'green', 'red', 'blue', 'orange'];
    var pos = 1;
    var ignoreHideAll = true;

    $('.topic-box h2').each(function(){
        $(this).addClass('grad'+pos);
        pos++;
    });
    ...
    $('.topic-box, .topic-box h2').mouseenter(function(){
        $(this).find('ul').show();
        ignoreHideAll = true;
    });

    $('.topic-box, .topic-box h2').mousemove(function(){
        $(this).find('ul').show();
        ignoreHideAll = true;
    });

    $('.topic-box ul').mouseleave(function(){
        $(this).hide();
        ignoreHideAll = false;
    });

    $('.home-page-topics').mouseleave(function(){
        setTimeout(hideAllTopicList, 450);
    })

    function hideAllTopicList() {
        if (ignoreHideAll) {
            $('.topic-box ul').hide();
        }
    }
    
    $('#block-views-ministry_news_ticker-block_1 ul').after('<div class="more-link"><a href="/all-news">All News</a></div>');
    
    $('#block-views-news_ticker-block_1 ul').after('<div class="more-link"><a href="/news">সকল খবর  </a></div>');
    ...
    $('body').addClass('npf-menu');
    $('#npf-menu').css('position', 'fixed');

    // Hover emulation for IE 6.
    if ($.browser.msie && parseInt(jQuery.browser.version) == 6) {
        $('#npf-menu li').hover(function() {
